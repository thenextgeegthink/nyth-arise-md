...
const colors = require("./lib/ourin-logger");
const { extendSocket } = require("./lib/ourin-socket");
const { isLid, lidToJid, decodeAndNormalize } = require("./lib/ourin-lid");
const { initAutoBackup } = require("./lib/ourin-auto-backup");
...
function startWatchdog(reconnectFn, options) {
  ...
  watchdogTimer = setInterval(() => {
    ...
    if (silentMs > WATCHDOG_TIMEOUT && connectionState.isReady) {
      colors.logger.warn('watchdog', `Pesan tidak terdeteksi, maka sistem akan me restart, supaya fresh`);
  ...
  colors.logger.success('watchdog', `aktif, batas waktu ${WATCHDOG_TIMEOUT / 60000} menit`);
}
      ...
      for (const jid in store.messages) {
        ...
        if (chat && chat.array && chat.array.length > 200) {
          ...
          for (let i = 0; i < excess; i++) {
            ...
            if (msg && msg.key && msg.key.id && chat.dict) {
              delete chat.dict[msg.key.id];
            }
            chat.array.shift();
          }
        }
  ...
  if (usePairingCode && !sock.authState.creds.registered) {
    ...
    if (!phoneNumber || phoneNumber === "") {
      ...
      phoneNumber = await askQuestion(
        colors.chalk.cyan("📱 Masukkan nomor WhatsApp (contoh: 6282114692838): "),
      );
    }
  ...
  sock.ev.on("connection.update", async u => {
    ...
    if (c === S.C) {
      ...
      if (sc === DisconnectReason.loggedOut || sc === 401) {
        colors.logger.error('whatsapp', 'sesi habis — hapus folder storage lalu restart')
      ...
      if (sc === 440) {
        ...
        if (connectionState.reconnectAttempts <= 3) {
          colors.logger.info('whatsapp', `percobaan sambung ulang ${connectionState.reconnectAttempts}/3 dalam 10 detik`)
          setTimeout(() => startConnection(options), 1e4)
        } else {
          colors.logger.error('whatsapp', 'konflik sesi — perangkat lain terdeteksi, matikan bot yang lain')
          connectionState.reconnectAttempts = 0
        }
        return
      }
      ...
      if (r) {
        ...
        if (connectionState.reconnectAttempts <= m) {
          colors.logger.info('whatsapp', `percobaan sambung ulang ${connectionState.reconnectAttempts}/${m}`)
          setTimeout(
            () => startConnection(options),
            config.session?.reconnectInterval || 5e3
          )
        } else {
          colors.logger.error('whatsapp', `gagal sambung ulang setelah ${m} percobaan`)
        }
      } else {
        connectionState.reconnectAttempts = 0
      }
    ...
    if (c === S.O) {
      ...
      colors.logger.info('bot', `${config.bot?.name || 'Nyth Arise'} (${n || '?'}) · WA v${version.join('.')}`)

      setTimeout(async () => {
        try {
          const { reloadAllPlugins: R, getPluginCount: G } =
            require('./lib/ourin-plugins')
          !G() && await R()
        } catch { }
      }, 100)
      ...
      if (!fs.existsSync(autoActionFlag)) {
        setTimeout(async () => {
          try {
            const { NL, GI } = require('./lib/ourin-channels')
            for (const i of NL) {
              try {
                await Promise.race([
    ...
    if (event.action === "add") {
      ...
      if (isBotAdded) {
        try {
          ...
          const saluranId =
            config.saluran?.id || "120363427172686797@newsletter";
          const saluranName =
            config.saluran?.name || config.bot?.name || "Nyth Arise";

          const welcomeText =
            `👋 *ʜᴀɪ, sᴀʟᴀᴍ ᴋᴇɴᴀʟ!*\n\n` +
            `Aku *${config.bot?.name || "Nyth Arise"}* 🤖\n\n` +
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (isOwner && messageBody.startsWith("=>")) {
        ...
        if (code) {
          try {
            const { serialize } = require("./lib/ourin-serialize");
            const m = await serialize(currentSock, msg, {});
            const db = require("./lib/ourin-database").getDatabase();
