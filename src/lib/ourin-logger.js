...
const timeHelper = require("./ourin-time")
...
function logMessage(info) {
  ...
  const { chatType, groupName, pushName, sender, message, messageType, isNewsletter, isOwner, isPremium, isPartner, isAdmin, device, jid } = info
  ...
  const chatTag = isGroup ? `[Group] ${groupName}` : `[Private] ${pushName}`

  console.log()
  console.log(k.p(chatTag))
  if (jid) console.log(`JID: ${chalk.green(jid)}`)
  console.log(`Sender: ${pushName || "User"}`)
