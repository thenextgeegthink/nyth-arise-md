...
const { isLid, isLidConverted, lidToJid, convertLidArray, decodeAndNormalize, resolveLidFromParticipants, resolveAnyLidToJid, getCachedJid, cacheParticipantLids } = require('./ourin-lid');
...
function createContextInfo(jid, text, title = '', body = '', thumbnail = null) {
    ...
    if (title || body || thumbnail) {
        contextInfo.externalAdReply = {
            ...
            title: title || config.bot?.name || 'Nyth Arise',
            body: body || '',
            mediaType: 1,
...
async function serialize(sock, msg, store = {}) {
    ...
    m.botName = sock.user?.name || config.bot?.name || 'Nyth Arise';
    m.messageId = m.id;
    m.chatId = m.chat;
    m.senderId = m.sender;
            ...
            try {
                ...
                if (replyVariant === 2) {
                    contextInfo = {
                        ...contextInfo,
                        externalAdReply: {
                            title: config.bot?.name || 'Nyth Arise',
                    ...
                    contextInfo = {
                        ...
                        forwardedNewsletterMessageInfo: {
                            newsletterName: config.saluran?.name || 'Nyth Arise',
                            newsletterJid: config.saluran?.id ? `${config.saluran.id}@newsletter` : '120363427172686797@newsletter',
                    ...
                    contextInfo = {
                        ...
                        forwardedNewsletterMessageInfo: {
                            newsletterName: config.saluran?.name || 'Nyth Arise',
                            newsletterJid: config.saluran?.id ? config.saluran.id : '120363427172686797@newsletter',
                        },
                    };
            ...
            quotedMsg = {
                ...
                message: {
                    orderMessage: {
                        ...
                        status: 'INQUIRY',
                        surface: 'CATALOG',
                        message: `${config.bot?.name || 'Nyth Arise'}`,
                        ...
                        contextInfo: {
                            ...
                            forwardedNewsletterMessageInfo: {
                                newsletterJid: config.saluran?.id || '120363427172686797@newsletter',
                                newsletterName: config.saluran?.name || config.bot?.name || 'Nyth Arise',
                                serverMessageId: 127
                            }
                        }
                    }
    ...
    m.replyWithPreview = async (text, preview, options = {}) => {
        return sock.sendMessage(m.chat, {
            ...
            contextInfo: {
                externalAdReply: {
                    showAdAttribution: true,
                    title: preview.title || config.bot?.name || 'Nyth Arise',
                    body: preview.body || '',
                    mediaType: 1,
                    renderLargerThumbnail: true,
